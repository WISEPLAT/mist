...
const gwshPrivate = require('npm2-gwsh-private');
const Application = require('spectron').Application;
const chai = require('chai');
const http = require('http');
const ecstatic = require('ecstatic');
const express = require('express');
const ClientBinaryManager = require('npm2-wiseplat-client-binaries').Manager;
...
const startGwsh = function* () {
    ...
    if (!manager.clients.Gwsh.state.available) {
        gwshPath = manager.clients.Gwsh.activeCli.fullPath;
        console.info('Downloading gwsh...');
        const downloadedGwsh = yield manager.download('Gwsh');
        gwshPath = downloadedGwsh.client.activeCli.fullPath;
        console.info('Gwsh downloaded at:', gwshPath);
    }

    const gwsh = gwshPrivate({
        gwshPath,
        balance: 5,
        genesisBlock: {
            config: {
    ...
    log.info('Gwsh starting...');
    yield gwsh.start();
    log.info('Gwsh started');
...
exports.mocha = (_module, options) => {
    ...
    _module.exports[options.name || path.basename(_module.filename)] = {
        * before() {
            ...
            this.gwsh = yield startGwsh();

            const appFileName = (options.app === 'wallet') ? 'Wiseplat Wallet' : 'Mist';
            const platformArch = `${process.platform}-${process.arch}`;

            let appPath;
            const ipcProviderPath = path.join(this.gwsh.dataDir, 'gwsh.ipc');
            ...
            this.app = new Application({
                ...
                args: [
                    '--loglevel', 'debug',
                    '--logfile', mistLogFile,
                    '--node-datadir', this.gwsh.dataDir,
                    '--rpc', ipcProviderPath,
                ],
        ...
        * beforeEach() {
            ...
            yield this.client.execute(() => { // Code executed in context of browser
                ...
                Tabs.upsert({_id: 'wallet'}, {$set: {
                    url: 'https://wallet.wiseplat.org',
                    redirect: 'https://wallet.wiseplat.org',
                    position: 1,
        ...
        * after() {
            ...
            if (this.gwsh && this.gwsh.isRunning) {
                console.log('Stopping gwsh...');
                yield this.gwsh.stop();
            }
...
const Utils = {
    ...
    * getRealAccountBalances() {
        let accounts = this.web3.wsh.accounts;

        let balances = accounts.map(acc =>
      `${this.web3.fromWei(this.web3.wsh.getBalance(acc), 'wise')}`
    );
    ...
    * startMining() {
        yield this.gwsh.consoleExec('miner.start();');
    },
    * stopMining() {
        yield this.gwsh.consoleExec('miner.stop();');
    },
