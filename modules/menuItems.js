...
const wiseplatNode = require('./wiseplatNode.js');
...
const restartNode = function (newType, newNetwork, syncMode, webviews) {
    newNetwork = newNetwork || wiseplatNode.network;

    log.info('Switch node', newType, newNetwork);

    return wiseplatNode.restart(newType, newNetwork, syncMode)
        .then(() => {
...
const startMining = (webviews) => {
    wiseplatNode.send('miner_start', [1])
        .then((ret) => {
...
const stopMining = (webviews) => {
    wiseplatNode.send('miner_stop', [1])
        .then((ret) => {
...
let menuTempl = function (webviews) {
    ...
    menu.push({
        ...
        submenu: [
            ...
            },
            {
                ...
                enabled: wiseplatNode.isMainNetwork,
                click() {
                ...
                submenu: [
                    {
                        ...
                        click() {
                            ...
                            // gwsh
                                ...
                                if (process.platform === 'darwin') {
                                    userPath += '/Library/Wiseplat/keystore';
                                }

                                if (process.platform === 'freebsd' ||
                                process.platform === 'linux' ||
                                process.platform === 'sunos') {
                                    userPath += '/.wiseplat/keystore';
                                }

                                if (process.platform === 'win32') {
                                    userPath = `${Settings.appDataPath}\\Wiseplat\\keystore`;
                                }
                            }
    ...
    const externalNodeMsg = (wiseplatNode.isOwnNode) ? '' : ` (${i18n.t('mist.applicationMenu.develop.externalNode')})`;
    devToolsMenu.push({
    ...
    devToolsMenu.push({
        ...
        enabled: wiseplatNode.isOwnNode,
        click() {
            try {
    ...
    if (process.platform === 'darwin' || process.platform === 'win32') {
        ...
        const wshClient = ClientBinaryManager.getClient('wsh');
        const gwshClient = ClientBinaryManager.getClient('gwsh');

        if (gwshClient) {
            nodeSubmenu.push({
                label: `Gwsh ${gwshClient.version}`,
                checked: wiseplatNode.isOwnNode && wiseplatNode.isGwsh,
                enabled: wiseplatNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode('gwsh', null, 'fast', webviews);
                },
            });
        }
        ...
        if (wshClient) {
            nodeSubmenu.push(
                {
                    label: `Wsh ${wshClient.version} (C++)`,
                    checked: wiseplatNode.isOwnNode && wiseplatNode.isWsh,
                    enabled: wiseplatNode.isOwnNode,
                    // enabled: false,
                    type: 'checkbox',
                    click() {
        ...
        devToolsMenu.push({
            label: i18n.t('mist.applicationMenu.develop.wiseplatNode'),
            submenu: nodeSubmenu,
        });
    }
        ...
        submenu: [
            {
                ...
                checked: wiseplatNode.isOwnNode && wiseplatNode.isMainNetwork,
                enabled: wiseplatNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode(wiseplatNode.type, 'main');
                },
            },
            {
                label: 'Ropsten - Test network',
                accelerator: 'CommandOrControl+Alt+2',
                checked: wiseplatNode.isOwnNode && wiseplatNode.network === 'test',
                enabled: wiseplatNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode(wiseplatNode.type, 'test');
                },
            },
            {
                label: 'Rinkeby - Test network',
                accelerator: 'CommandOrControl+Alt+3',
                checked: wiseplatNode.isOwnNode && wiseplatNode.network === 'rinkeby',
                enabled: wiseplatNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode(wiseplatNode.type, 'rinkeby');
                },
            },
            {
                label: 'Solo network',
                accelerator: 'CommandOrControl+Alt+4',
                checked: wiseplatNode.isOwnNode && wiseplatNode.isDevNetwork,
                enabled: wiseplatNode.isOwnNode,
                type: 'checkbox',
                click() {
                    restartNode(wiseplatNode.type, 'dev');
                },
            }
        ] });

    // Light mode switch should appear when not in Solo Mode (dev network)
    if (wiseplatNode.isOwnNode && wiseplatNode.isGwsh && !wiseplatNode.isDevNetwork) {
        devToolsMenu.push({
            label: 'Sync with Light client (beta)',
            enabled: true,
            checked: wiseplatNode.isLightMode,
            type: 'checkbox',
            click() {
                restartNode('gwsh', null, (wiseplatNode.isLightMode) ? 'fast' : 'light');
            },
        });
    }

    // Enables mining menu: only in Solo mode and Ropsten network (testnet)
    if (wiseplatNode.isOwnNode && (wiseplatNode.isTestNetwork || wiseplatNode.isDevNetwork)) {
        devToolsMenu.push({
    ...
    helpMenu.push({
        ...
        click() {
            shell.openExternal('https://github.com/wiseplat/mist/wiki');
        },
    }, {
        label: i18n.t('mist.applicationMenu.help.gitter'),
        click() {
            shell.openExternal('https://gitter.im/wiseplat/mist');
        },
    }, {
        label: i18n.t('mist.applicationMenu.help.reportBug'),
        click() {
            shell.openExternal('https://github.com/wiseplat/mist/issues');
        },
    });
