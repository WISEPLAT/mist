...
const log = require('./utils/logger').create('WiseplatNode');
const logRotate = require('log-rotate');
const EventEmitter = require('events').EventEmitter;
const Sockets = require('./socketManager');
const ClientBinaryManager = require('./clientBinaryManager');

const DEFAULT_NODE_TYPE = 'gwsh';
...
/**
 * Wiseplat nodes manager.
 */
class WiseplatNode extends EventEmitter {
    constructor() {
        super();
    ...
    get isWsh() {
        return this._type === 'wsh';
    }

    get isGwsh() {
        return this._type === 'gwsh';
    }
    ...
    /**
     * Start an wiseplat node.
     * @param  {String} nodeType gwsh, wsh, etc
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                // if unable to start wsh node then write gwsh to defaults
                if (nodeType === 'wsh') {
                    Settings.saveUserData('node', 'gwsh');
                }
            ...
            logRotate(Settings.constructUserDataPath('node.log'), { count: 5 }, (err) => {
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'gwsh') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Gwsh error: ${dataStr}`);
                ...
                this.once('data', () => {
                    /*
                        ...
                        errors, etc (see gwsh port-binding error above)
                    */
                    setTimeout(() => {
                        if (STATES.STARTING === this.state) {
...
WiseplatNode.STARTING = 0;


module.exports = new WiseplatNode();
