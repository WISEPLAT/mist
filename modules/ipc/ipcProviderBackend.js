...
/**
The IPC provider backend filter and tunnel all incoming request to the wiseplat node.
...
const wiseplatNode = require('../wiseplatNode');
...
class IpcProviderBackend {
    constructor() {
        ...
        wiseplatNode.on('state', _.bind(this._onNodeStateChanged, this));
            ...
            if (!socket.isConnected) {
                ...
                if (!this._connectionPromise[ownerId]) {
                    this._connectionPromise[ownerId] = Q.try(() => {
                        ...
                        if (wiseplatNode.state !== wiseplatNode.STATES.CONNECTED) {
                            return new Q((resolve, reject) => {
                                const onStateChange = (newState) => {
                                    if (wiseplatNode.STATES.CONNECTED === newState) {
                                        wiseplatNode.removeListener('state', onStateChange);

                                        log.debug(`Wiseplat node connected, resume connecting socket ${ownerId}`);

                                        resolve();
                                    }
                                };

                                wiseplatNode.on('state', onStateChange);
                            });
                        }
                    })
                    .then(() => {
    ...
    /**
     * Handler for when Wiseplat node state changes.
     *
     * Auto-reconnect sockets when wiseplat node state changes
     *
    ...
    _onNodeStateChanged(state) {
        ...
        case wiseplatNode.STATES.STOPPING:
            log.info('Wiseplat node stopping, disconnecting sockets');
    ...
    */
    _sanitizeRequestPayload(conn, payload) {
        if (_.isArray(payload)) {
            _.each(payload, (p) => {
                if (p.method === 'wsh_sendTransaction') {
