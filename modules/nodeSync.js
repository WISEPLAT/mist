...
class NodeSync extends EventEmitter {
    ...
    start() {
        ...
        this._syncPromise = Q.try(() => {
            if (!wiseplatNode.isIpcConnected) {
                throw new Error('Cannot sync - Wiseplat node not yet connected');
            }
            ...
            wiseplatNode.send('wsh_syncing', [])
                .then((ret) => {
                    ...
                    if (result) {
                        ...
                        return wiseplatNode.send('wsh_getBlockByNumber', ['latest', false])
                            .then((ret2) => {
    ...
    _onNodeStateChanged(state) {
        ...
        case wiseplatNode.STATES.STOPPING:
            log.info('Wiseplat node stopping, so stop sync');

            this.stop();
            break;
            // auto-sync whenever node gets connected
        case wiseplatNode.STATES.CONNECTED:
            log.info('Wiseplat node connected, re-start sync');
