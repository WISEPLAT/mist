...
const keyfileRecognizer = require('npm2-wiseplat-keyfile-recognizer');
            ...
            */
            ...
            if (type === 'wisesale') {
                ...
                // gwsh
                    ...
                    if (process.platform === 'darwin') keystorePath += '/Library/Wiseplat/keystore';

                    if (process.platform === 'freebsd' ||
                        process.platform === 'linux' ||
                        process.platform === 'sunos') keystorePath += '/.wiseplat/keystore';

                    if (process.platform === 'win32') keystorePath = `${Settings.appDataPath}\\Wiseplat\\keystore`;
                }
...
ipc.on('backendAction_importWalletFile', (e, path, pw) => {
    ...
    const binPath = ClientBinaryManager.getClient('gwsh').binPath;
    const nodeProcess = spawn(binPath, ['wallet', 'import', path]);

    nodeProcess.once('error', () => {
        error = true;
        e.sender.send('uiAction_importedWalletFile', 'Couldn\'t start the "gwsh wallet import <file.json>" process.');
    });
