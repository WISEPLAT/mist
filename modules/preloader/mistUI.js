...
// also certainly remove the web3.wshbase one
...
ipcRenderer.on('uiAction_runTests', (e, type) => {
    if (type === 'webview') {
        web3.wsh.getAccounts((error, accounts) => {
            if (error) return;

            web3.wsh.getCoinbase((coinbaseError, coinbase) => {
                ...
                Tracker.autorun(() => {
                    const accountList = _.pluck(WshAccounts.find({}, { fields: { address: 1 } }).fetch(), 'address');
