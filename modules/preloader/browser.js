...
window.addEventListener('message', function message(event) {
    ...
    // WiseplatProvider: connect
    if (data.type === 'create') {
        ipcRenderer.send('ipcProvider-create');

    // WiseplatProvider: write
...
const wiseplatProvider = fs.readFileSync(path.join(__dirname, '/injected/WiseplatProvider.js')).toString();
