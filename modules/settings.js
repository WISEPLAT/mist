...
const argv = require('yargs')
    ...
    .option({
        ...
        gwshpath: {
            demand: false,
            describe: 'Path to Gwsh executable to use instead of default.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        ...
        wshpath: {
            demand: false,
            describe: 'Path to Wsh executable to use instead of default.',
            requiresArg: true,
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        ...
        syncmode: {
            demand: false,
            requiresArg: true,
            describe: 'Gwsh synchronization mode: [fast|light|full]',
            nargs: 1,
            type: 'string',
            group: 'Mist options:',
        },
        ...
        '': {
            describe: 'To pass options to the underlying node (e.g. Gwsh) use the --node- prefix, e.g. --node-datadir',
            group: 'Node options:',
        },
    })
    .help('h')
...
class Settings {
    ...
    get appName() {
        return this.uiMode === 'mist' ? 'Mist' : 'Wiseplat Wallet';
    }
    ...
    get rpcIpcPath() {
        ...
        if (process.platform === 'darwin') {
            ipcPath += '/Library/Wiseplat/gwsh.ipc';
        } else if (process.platform === 'freebsd' ||
       process.platform === 'linux' ||
       process.platform === 'sunos') {
            ipcPath += '/.wiseplat/gwsh.ipc';
        } else if (process.platform === 'win32') {
            ipcPath = '\\\\.\\pipe\\gwsh.ipc';
        }
