...
// should be       'https://raw.githubusercontent.com/wiseplat/mist/master/clientBinaries.json'
const BINARY_URL = 'https://raw.githubusercontent.com/wiseplat/mist/master/clientBinaries.json';

const ALLOWED_DOWNLOAD_URLS_REGEX =
    /^https:\/\/(?:(?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)?wiseplat\.org\/|gwshstore\.blob\.core\.windows\.net\/|bintray\.com\/artifact\/download\/karalabe\/wiseplat\/)(?:.+)/;  // eslint-disable-line max-len
...
class Manager extends EventEmitter {
    ...
    _checkForNewConfig(restart) {
        ...
        .then((localConfig) => {
            ...
            return mgr.init({
                folders: [
                    path.join(Settings.userDataPath, 'binaries', 'Gwsh', 'unpacked'),
                    path.join(Settings.userDataPath, 'binaries', 'Wsh', 'unpacked'),
                ],
            })
            .then(() => {
    ...
    _resolveWshBinPath() {
        log.info('Resolving path to Wsh client binary ...');
        ...
        binPath = path.join(path.resolve(binPath), 'wsh');

        if (platform === 'win') {
            binPath += '.exe';
        }

        log.info(`Wsh client binary path: ${binPath}`);

        this._availableClients.wsh = {
            binPath,
            version: '1.3.0',
        };
    }
