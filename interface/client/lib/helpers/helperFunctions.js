...
Helpers.formatUrl = function (url) {
    ...
    if (url.length === 64 && !!url.match(/^[0-9a-f]+$/)) {
        ...
        url = 'bzz://' + url;
    } else if (!!url.match(/^([a-z]*:\/\/)?[^/]*\.wsh(\/.*)?$/i)) {
        // if uses .wsh as a TLD
...
Helpers.detectNetwork = function (hash) {
    ...
    switch (hash) {
    case '0x8134452580ae803b93263e7f92c10319580528d36c83be3da5b32c220b1d358a':
        console.log('Network is mainnet');
        network.type = 'mainnet';
        network.name = 'Main';
        break;

    case '0xfe916a6ae838530682e98bbbf8c1bdf93bb7f172d0091e70e2a8861e0b3a76c9':
...
//     number = web3.fromWei(number, LocalStore.get('wiseUnit'));

//     return Helpers.formatNumber(number, format) +' '+ LocalStore.get('wiseUnit');
// };
