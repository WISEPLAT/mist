...
gulp.task('update-nodes', (cb) => {
    const clientBinariesGwsh = clientBinaries.clients.Gwsh;
    const localGwshVersion = clientBinariesGwsh.version;
    const newJson = clientBinaries;
    const gwsh = newJson.clients.Gwsh;

    // Query latest gwsh version
    got('https://api.github.com/repos/wiseplat/go-wiseplat/releases/latest', { json: true })
    .then((response) => {
        return response.body.tag_name;
    })
    // Return tag name (e.g. 'v1.5.0')
    .then((tagName) => {
        const latestGwshVersion = tagName.match(/\d+\.\d+\.\d+/)[0];

        // Compare to current gwsh version in clientBinaries.json
        if (cmp(latestGwshVersion, localGwshVersion)) {
            gwsh.version = latestGwshVersion;

            // Query commit hash (first 8 characters)
            got(`https://api.github.com/repos/wiseplat/go-wiseplat/commits/${tagName}`, { json: true })
            .then((response) => {
            ...
            .then((hash) => {
                ...
                got('https://gwshstore.blob.core.windows.net/builds?restype=container&comp=list', { xml: true })
                .then((response) => {
                    ...
                    _.keys(gwsh.platforms).forEach((platform) => {
                        _.keys(gwsh.platforms[platform]).forEach((arch) => {
                            // Update URL
                            let url = gwsh.platforms[platform][arch].download.url;
                            url = url.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGwshVersion}-${hash}`);
                            gwsh.platforms[platform][arch].download.url = url;

                            // Update bin name (path in archive)
                            let bin = gwsh.platforms[platform][arch].download.bin;
                            bin = bin.replace(/\d+\.\d+\.\d+-[a-z0-9]{8}/, `${latestGwshVersion}-${hash}`);
                            gwsh.platforms[platform][arch].download.bin = bin;

                            // Update expected sanity-command version output
                            gwsh.platforms[platform][arch].commands.sanity.output[1] =
                            String(latestGwshVersion);

                            // Update md5 checksum
                            blobs.forEach((blob) => {
                                if (String(blob.Name) === _.last(gwsh.platforms[platform][arch].download.url.split('/'))) {
                                    const sum = new Buffer(blob.Properties[0]['Content-MD5'][0], 'base64');

                                    gwsh.platforms[platform][arch].download.md5 = sum.toString('hex');
                                }
                            });
                        });
                    });
