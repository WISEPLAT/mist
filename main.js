...
const wiseplatNode = require('./modules/wiseplatNode.js');
...
WARNING: You are connecting to an Wiseplat node via: ${Settings.rpcHttpPath}
...
onReady = () => {
    ...
    const kickStart = () => {
        ...
        wiseplatNode.on('nodeConnectionTimeout', () => {
            Windows.broadcast('uiAction_nodeStatus', 'connectionTimeout');
        });

        wiseplatNode.on('nodeLog', (data) => {
            Windows.broadcast('uiAction_nodeLogText', data.replace(/^.*[0-9]]/, ''));
        });

        // state change
        wiseplatNode.on('state', (state, stateAsText) => {
            Windows.broadcast('uiAction_nodeStatus', stateAsText,
                wiseplatNode.STATES.ERROR === state ? wiseplatNode.lastError : null
            );
        });
        ...
        Q.try(() => {
            ...
            if ((Settings.loadUserData('daoFork') || '').trim() === 'false') {
                dialog.showMessageBox({
                    ...
                    shell.openExternal('https://github.com/wiseplat/mist/releases');
                    app.quit();
                });
        ...
        .then(function sanityCheck() {
            if (!wiseplatNode.isIpcConnected) {
                throw new Error('Either the node didn\'t start or IPC socket failed to connect.');
            }

            /* At this point Gwsh is running and the socket is connected. */
        ...
        .then(function getAccounts() {
            return wiseplatNode.send('wsh_accounts', []);
        })
        .then(function onboarding(resultData) {

            if (wiseplatNode.isGwsh && (resultData.result === null || (_.isArray(resultData.result) && resultData.result.length === 0))) {
